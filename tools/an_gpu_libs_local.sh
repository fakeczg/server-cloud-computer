...
flatten_copy_to_gerrit() {
	...
	*)
		echo "未知架构：$arch（应为 aarch64 或 armv7-a）"
		exit 1
		;;
	esac
...
delete-root)
	...
	if [ "$DEL_ALL" -eq 1 ]; then
		...
		armv7-a) DEL32=1 ;;
		"")
			echo "delete-root 需要指定 --arch aarch64|armv7-a 或者使用 --all"
			exit 1
			;;
		*)
			echo "未知架构：$ARCH"
...
delete-src)
	...
	if [ "$DEL_ALL" -eq 1 ]; then
		...
		armv7-a) DEL32=1 ;;
		"")
			echo "delete-src 需要指定 --arch aarch64|armv7-a 或使用 --all"
			exit 1
			;;
		*)
			echo "未知架构：$ARCH"
...
times-gerrit)
	...
	if [ "$DEL_ALL" -eq 1 ]; then
		...
		"")
			echo "times-gerrit 需要指定 --arch aarch64|armv7-a，或使用 --all"
			exit 1
			;;
		*)
			echo "未知架构：$ARCH"
